...
public class OpenRCServiceController: ServiceController
{
	...
	public override ServiceManager Install(ServiceDescription service)
	{
		...
		if (srvc.DependsOn != null && srvc.DependsOn.Any())
		{
        ...
        if (!string.IsNullOrEmpty(srvc.OutputLog)) body.AppendLine($"output_log=\"{srvc.OutputLog}\"");
        if (!string.IsNullOrEmpty(srvc.ErrorLog)) body.AppendLine($"error_log=\"{srvc.ErrorLog}\"");
