...
public class SystemdServiceController : ServiceController
{
	...
	public override OSService Info(string serviceId)
	{
		...
		string loaded = null, active = null, status = null;
		if (match.Success)
		{
		...
		return new OSService()
		{
            ...
            CanStop = loaded == "loaded" && active == "active" && status == "running",
            CanPauseAndContinue = loaded == "loaded" && active == "active" && status == "running",
        };
	}
	public override void ChangeStatus(string serviceId, OSServiceStatus status)
	{
		...
		if (state.Status == OSServiceStatus.Running)
		{
			...
			{
				Shell.Exec($"systemctl stop {serviceId}.service");
			}
		}
		else
		{
			...
			{
				Shell.Exec($"systemctl start {serviceId}.service");
			}
		}
	}
